...
func SetSystemProxy(port uint16, _ bool, bypasses ...string) (clearFunc func() error, err error) {
	...
	*((*uint32)(unsafe.Pointer(&flagsOption.value))) = proxyTypeProxy | proxyTypeDirect
	...
	proxyAddr := fmt.Sprint("127.0.0.1:", port)
	*((*uintptr)(unsafe.Pointer(&proxyOption.value))) = uintptr(unsafe.Pointer(windows.StringToUTF16Ptr(proxyAddr)))

	// 不代理本地地址
	bypass := "<local>"
	if len(bypasses) != 0 {
		bypass = strings.Join(bypasses, ";")
	}
	...
	*((*uintptr)(unsafe.Pointer(&bypassOption.value))) = uintptr(unsafe.Pointer(windows.StringToUTF16Ptr(bypass)))
	...
	}, setOptions(flagsOption, proxyOption, bypassOption)
}
